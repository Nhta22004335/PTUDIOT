...
// Get current data
$stmt = $conn->prepare("SELECT * FROM thietbi WHERE idtb = ?");
$stmt->execute([$id]);
$thietbi = $stmt->fetch(PDO::FETCH_ASSOC);
...
<body class="bg-gray-100 py-10">
    <div class="max-w-xl mx-auto bg-white p-8 rounded shadow">
        <h1 class="text-2xl font-bold mb-6">Cập Nhật Thiết Bị</h1>
        <form method="POST" class="space-y-4">
            <div>
                <label class="block mb-1 font-medium">Tên Thiết Bị</label>
                <input type="text" name="tentb" value="<?= htmlspecialchars($thietbi['tentb']) ?>" required class="w-full px-3 py-2 border rounded">
            </div>
            <div>
            ...
            <div>
                ...
                <select name="idvt" required class="w-full px-3 py-2 border rounded">
                    <?php foreach ($khuvucs as $kv): ?>
                        <option value="<?= $kv['idkv'] ?>" <?= $thietbi['idvt'] == $kv['idkv'] ? 'selected' : '' ?>><?= $kv['tenkv'] ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
